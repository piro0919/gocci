...
- **外付けの上の抜け殻を、アプリから外せるようにするか。** フルディスクアクセスを求めれば
  通る見込みはあるが未確認。メニューバーの小物にそれを要求するかは別の話。
  今は人に `umount -f` を頼む形にしてある
...
- **外付けの上の抜け殻は、アプリからは外せない。** `umount: Operation not permitted` が返る。
  同じ命令がターミナルからは通り、内蔵（`~/GocciHome`）に張った抜け殻はアプリからでも
  一発で外せた。外付けの上だけが拒まれる。ファイルの作成は外付けでも通るので、
  読み書きの権限ではなくアンマウントの権限。
  今は状態表示に `umount -f <パス>` を出して人に頼む形にしてある
