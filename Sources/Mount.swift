...
final class MountController {
    ...
    private func launch(
        ...
        task.arguments = [
            ...
            // 「その名前は無い」という答えを macOS に覚えさせない。
            //
            // 既定の NFS クライアントは negnamecache が有効で、一度無いと答えた名前を、
            // そのフォルダの更新時刻が変わるまで覚え続ける。rclone の VFS は中身が増えても
            // フォルダの時刻を動かさないので、他の端末で足したファイルに手が届かなくなる。
            // 一覧（readdir）には出るのに、その道を直接開こうとすると「無い」と言われる。
            //
            // 実測（2026-08-15）: これを外すと 50 秒で見えるようになる。付けたままだと
            // 400 秒たっても見えない。50 秒は --poll-interval の 60 秒と釣り合う
            "-o", "nonegnamecache",
        ]
