#!/bin/bash
# Gocci を1つ上げる。DMG を作り、更新情報に署名して、GitHub Releases に置く。
#
#   ./release.sh 1.0.1
#
...
REPO="piro0919/gocci"
APP="Gocci.app"
DMG="Gocci-${VERSION}.dmg"
ZIP="Gocci-${VERSION}.zip"

# 版数を Info.plist に焼き込むため、build.sh へ渡す
GOCCI_VERSION="$VERSION" ./build.sh
...
hdiutil create -quiet -volname "Gocci" -srcfolder "$STAGE" -ov -format UDZO "dist/$DMG"
rm -rf "$STAGE"
