...
// 自動更新。Konechi のものをそのまま持ってきた。
//
// 確認は起動時に1回だけで、定期的には見に行かない。見つかったときだけ画面を出し、
// 入れるかどうかを毎回尋ねる。
//
...
final class Updater: NSObject, SPUUpdaterDelegate {
    ...
    func updater(_ updater: SPUUpdater, didFindValidUpdate item: SUAppcastItem) {
        logger.info("更新が見つかった: \(item.displayVersionString, privacy: .public)")
        foundUpdate = true
    }
    ...
    func updater(
        ...
        logger.info("更新の確認が終わった: \(error.map { "\($0)" } ?? "問題なし", privacy: .public)")
