...
今どれだけ手元に置いているかが、どこにも出ていなかった。21GB まで膨らんだことに気付くのが
遅れたのはそのため。上限と並べて出す（`キャッシュ 100MB / 50GB`）。

### 取得中のものをメニューに出す

裏で1本ずつ落としているので、Finder のバッジだけでは動いているのか止まっているのかが
分からない。開いたときに、進んでいるものから5件と、残りの件数を出す。
行を押すと Finder でその場所を開く。

**出すのは実際に取りにいく対象だけ。** Finder が覗いただけのものは含めない。
含めると「取得中」と出たまま何も起きず、実態と合わない（バッジ側でも雲として扱っている）。
